// Emoji folosite în site. Se afișează din SVG-uri locale (Noto Emoji, Apache-2.0),
// ca să arate la fel pe toate dispozitivele. Lista de coduri e folosită și de tools/fetch_assets.py.

import { escapeHTML } from '../core/dom.js';

export const EMOJI = {
  mar: { char: '🍎', code: '1f34e', label: 'măr' },
  para: { char: '🍐', code: '1f350', label: 'pară' },
  cirese: { char: '🍒', code: '1f352', label: 'cireșe' },
  pepene: { char: '🍉', code: '1f349', label: 'pepene' },
  morcov: { char: '🥕', code: '1f955', label: 'morcov' },
  stea: { char: '⭐', code: '2b50', label: 'stea' },
  luna: { char: '🌙', code: '1f319', label: 'lună' },
  soare: { char: '☀️', code: '2600', label: 'soare' },
  nor: { char: '☁️', code: '2601', label: 'nor' },
  racheta: { char: '🚀', code: '1f680', label: 'rachetă' },
  planeta: { char: '🪐', code: '1fa90', label: 'planetă' },
  astronaut: { char: '🧑‍🚀', code: '1f9d1_200d_1f680', label: 'astronaut' },
  valiza: { char: '🧳', code: '1f9f3', label: 'valiză' },
  scoica: { char: '🐚', code: '1f41a', label: 'scoică' },
  inghetata: { char: '🍦', code: '1f366', label: 'înghețată' },
  plaja: { char: '🏖️', code: '1f3d6', label: 'plajă' },
  termometru: { char: '🌡️', code: '1f321', label: 'termometru' },
  cos: { char: '🧺', code: '1f9fa', label: 'coș' },
  ou: { char: '🥚', code: '1f95a', label: 'ou' },
  branza: { char: '🧀', code: '1f9c0', label: 'brânză' },
  rosie: { char: '🍅', code: '1f345', label: 'roșie' },
  ceas: { char: '⏰', code: '23f0', label: 'ceas' },
  bani: { char: '💰', code: '1f4b0', label: 'bani' },
  vaca: { char: '🐄', code: '1f404', label: 'vacă' },
  oaie: { char: '🐑', code: '1f411', label: 'oaie' },
  gaina: { char: '🐔', code: '1f414', label: 'găină' },
  cal: { char: '🐴', code: '1f434', label: 'cal' },
  porc: { char: '🐷', code: '1f437', label: 'porc' },
  iepure: { char: '🐰', code: '1f430', label: 'iepure' },
  pisica: { char: '🐱', code: '1f431', label: 'pisică' },
  caine: { char: '🐶', code: '1f436', label: 'câine' },
  rata: { char: '🦆', code: '1f986', label: 'rață' },
  lup: { char: '🐺', code: '1f43a', label: 'lup' },
  vulpe: { char: '🦊', code: '1f98a', label: 'vulpe' },
  urs: { char: '🐻', code: '1f43b', label: 'urs' },
  arici: { char: '🦔', code: '1f994', label: 'arici' },
  miere: { char: '🍯', code: '1f36f', label: 'miere' },
  lapte: { char: '🥛', code: '1f95b', label: 'lapte' },
  veverita: { char: '🐿️', code: '1f43f', label: 'veveriță' },
  bravo: { char: '👏', code: '1f44f', label: 'bravo' },
  idee: { char: '💡', code: '1f4a1', label: 'idee' },
  calcul: { char: '🧮', code: '1f9ee', label: 'socotitoare' },
  capcana: { char: '⚠️', code: '26a0', label: 'atenție' },
  proba: { char: '🔍', code: '1f50d', label: 'verificare' },
  muschi: { char: '💪', code: '1f4aa', label: 'putere' },
  petrecere: { char: '🎉', code: '1f389', label: 'sărbătoare' },
  deget: { char: '👆', code: '1f446', label: 'deget' },
  trist: { char: '😟', code: '1f61f', label: 'îngrijorat' },
  neutru: { char: '😐', code: '1f610', label: 'așa și așa' },
  vesel: { char: '🙂', code: '1f642', label: 'mulțumit' },
  // obiecte (corpuri geometrice în viața reală)
  zar: { char: '🎲', code: '1f3b2', label: 'zar' },
  gheata: { char: '🧊', code: '1f9ca', label: 'cub de gheață' },
  minge: { char: '⚽', code: '26bd', label: 'minge' },
  glob: { char: '🌍', code: '1f30d', label: 'glob pământesc' },
  conserva: { char: '🥫', code: '1f96b', label: 'conservă' },
  baterie: { char: '🔋', code: '1f50b', label: 'baterie' },
  cutie: { char: '📦', code: '1f4e6', label: 'cutie' },
  carte: { char: '📕', code: '1f4d5', label: 'carte' },
  creion: { char: '✏️', code: '270f', label: 'creion' },
  rigla: { char: '📏', code: '1f4cf', label: 'riglă' },
  echer: { char: '📐', code: '1f4d0', label: 'echer' },
  // piață și legume
  salata: { char: '🥬', code: '1f96c', label: 'salată' },
  ardei: { char: '🫑', code: '1fad1', label: 'ardei' },
  castravete: { char: '🥒', code: '1f952', label: 'castravete' },
  suc: { char: '🧃', code: '1f9c3', label: 'suc' },
  // energie și anotimpuri
  vant: { char: '💨', code: '1f4a8', label: 'vânt' },
  apa: { char: '💧', code: '1f4a7', label: 'apă' },
  petrol: { char: '🛢️', code: '1f6e2', label: 'petrol' },
  foc: { char: '🔥', code: '1f525', label: 'foc' },
  fulg: { char: '❄️', code: '2744', label: 'fulg de nea' },
  frunze: { char: '🍂', code: '1f342', label: 'frunze de toamnă' },
  lalea: { char: '🌷', code: '1f337', label: 'lalea' },
  // oameni
  bunic: { char: '👴', code: '1f474', label: 'bunic' },
  bunica: { char: '👵', code: '1f475', label: 'bunică' },
  baiat: { char: '👦', code: '1f466', label: 'băiat' },
  fata: { char: '👧', code: '1f467', label: 'fată' },
  // oraș și transport
  tramvai: { char: '🚋', code: '1f68b', label: 'tramvai' },
  metrou: { char: '🚇', code: '1f687', label: 'metrou' },
  tren: { char: '🚆', code: '1f686', label: 'tren' },
  autobuz: { char: '🚌', code: '1f68c', label: 'autobuz' },
  masina: { char: '🚗', code: '1f697', label: 'mașină' },
  bilet: { char: '🎫', code: '1f3ab', label: 'bilet' },
  harta: { char: '🗺️', code: '1f5fa', label: 'hartă' },
  // magazin
  bon: { char: '🧾', code: '1f9fe', label: 'bon de casă' },
  'cos-cumparaturi': { char: '🛒', code: '1f6d2', label: 'coș de cumpărături' },
  magazin: { char: '🏪', code: '1f3ea', label: 'magazin' },
  robot: { char: '🤖', code: '1f916', label: 'robot' },
  puzzle: { char: '🧩', code: '1f9e9', label: 'puzzle' },
  cuburi: { char: '🧱', code: '1f9f1', label: 'cuburi de construit' },
  cana: { char: '☕', code: '2615', label: 'cană' },
  tricou: { char: '👕', code: '1f455', label: 'tricou' },
  ghete: { char: '👟', code: '1f45f', label: 'ghete' },
  rucsac: { char: '🎒', code: '1f392', label: 'rucsac' },
  // școală și sport
  scoala: { char: '🏫', code: '1f3eb', label: 'școală' },
  medalie: { char: '🏅', code: '1f3c5', label: 'medalie' },
  trofeu: { char: '🏆', code: '1f3c6', label: 'trofeu' },
  alergare: { char: '🏃', code: '1f3c3', label: 'copil care aleargă' },
  inot: { char: '🏊', code: '1f3ca', label: 'înot' },
  fotbal: { char: '⚽', code: '26bd', label: 'minge de fotbal' },
  baschet: { char: '🏀', code: '1f3c0', label: 'minge de baschet' },
  dans: { char: '💃', code: '1f483', label: 'dans' },
  cronometru: { char: '⏱️', code: '23f1', label: 'cronometru' },
  ecran: { char: '📺', code: '1f4fa', label: 'ecran' },
  // corp și sănătate
  os: { char: '🦴', code: '1f9b4', label: 'os' },
  dinte: { char: '🦷', code: '1f9b7', label: 'dinte' },
  pat: { char: '🛏️', code: '1f6cf', label: 'pat' },
  sapun: { char: '🧼', code: '1f9fc', label: 'săpun' },
  periuta: { char: '🪥', code: '1faa5', label: 'periuță de dinți' },
  sampon: { char: '🧴', code: '1f9f4', label: 'șampon' },
  // alimente
  banana: { char: '🍌', code: '1f34c', label: 'banană' },
  strugure: { char: '🍇', code: '1f347', label: 'struguri' },
  paine: { char: '🍞', code: '1f35e', label: 'pâine' },
  portocala: { char: '🍊', code: '1f34a', label: 'portocală' },
  prajitura: { char: '🍰', code: '1f370', label: 'prăjitură' },
  bomboane: { char: '🍬', code: '1f36c', label: 'bomboane' },
  // interfață
  sunet: { char: '🔊', code: '1f50a', label: 'sunete pornite' },
  mut: { char: '🔇', code: '1f507', label: 'sunete oprite' },
  // calcul fulger
  fulger: { char: '⚡', code: '26a1', label: 'fulger' },
  steag: { char: '🏁', code: '1f3c1', label: 'steag de sosire' },
  tinta: { char: '🎯', code: '1f3af', label: 'țintă' },
  familie: { char: '👪', code: '1f46a', label: 'familie' },
  lacat: { char: '🔒', code: '1f512', label: 'lacăt' },
  unelte: { char: '🛠️', code: '1f6e0', label: 'unelte' },
  aur: { char: '🥇', code: '1f947', label: 'medalie de aur' },
  argint: { char: '🥈', code: '1f948', label: 'medalie de argint' },
  bronz: { char: '🥉', code: '1f949', label: 'medalie de bronz' },
  // grafice și hărți (Jocuri fulger)
  grafic: { char: '📊', code: '1f4ca', label: 'grafic' },
  spital: { char: '🏥', code: '1f3e5', label: 'spital' },
  gara: { char: '🚉', code: '1f689', label: 'gară' },
  castel: { char: '🏰', code: '1f3f0', label: 'castel' },
  stadion: { char: '🏟️', code: '1f3df', label: 'stadion' },
  casa: { char: '🏠', code: '1f3e0', label: 'casă' },
  parc: { char: '🌳', code: '1f333', label: 'parc' },
  muzeu: { char: '🏛️', code: '1f3db', label: 'muzeu' },
  biblioteca: { char: '📚', code: '1f4da', label: 'bibliotecă' },
  circ: { char: '🎪', code: '1f3aa', label: 'circ' },
  roata: { char: '🎡', code: '1f3a1', label: 'roata mare' },
  fantana: { char: '⛲', code: '26f2', label: 'fântână' },
  pinguin: { char: '🐧', code: '1f427', label: 'pinguin' },
  bufnita: { char: '🦉', code: '1f989', label: 'bufniță' },
  peste: { char: '🐟', code: '1f41f', label: 'pește' },
  fluture: { char: '🦋', code: '1f98b', label: 'fluture' },
  albina: { char: '🐝', code: '1f41d', label: 'albină' },
  liliac: { char: '🦇', code: '1f987', label: 'liliac' },
  testoasa: { char: '🐢', code: '1f422', label: 'broască țestoasă' },
  papagal: { char: '🦜', code: '1f99c', label: 'papagal' },
  delfin: { char: '🐬', code: '1f42c', label: 'delfin' },
  leu: { char: '🦁', code: '1f981', label: 'leu' },
  tigru: { char: '🐯', code: '1f42f', label: 'tigru' },
  panda: { char: '🐼', code: '1f43c', label: 'panda' },
  maimuta: { char: '🐵', code: '1f435', label: 'maimuță' },
  koala: { char: '🐨', code: '1f428', label: 'koala' },
  capsuni: { char: '🍓', code: '1f353', label: 'căpșuni' },
  lamaie: { char: '🍋', code: '1f34b', label: 'lămâie' },
  ciocolata: { char: '🍫', code: '1f36b', label: 'ciocolată' },
  gogoasa: { char: '🍩', code: '1f369', label: 'gogoașă' },
  biscuit: { char: '🍪', code: '1f36a', label: 'biscuit' },
  sandvis: { char: '🥪', code: '1f96a', label: 'sandviș' },
  castana: { char: '🌰', code: '1f330', label: 'castană' },
  fasole: { char: '🫘', code: '1fad8', label: 'fasole' },
};

export const hasEmoji = (name) => Object.hasOwn(EMOJI, name);

export function emojiHTML(name, { cls = 'c-emoji' } = {}) {
  const e = EMOJI[name];
  if (!e) return escapeHTML(name);
  return `<img class="${cls}" src="assets/emoji/${e.code}.svg" alt="${escapeHTML(e.label)}" draggable="false">`;
}
