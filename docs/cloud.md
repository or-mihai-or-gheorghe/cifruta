...
| `…/state/fulger` | proprietarul, adminul | chei „temă:nivel”: `best` (recordul, cu `correct` și `bestStreak`) și `week` (cea mai bună rundă a săptămânii); `medals`: medaliile câștigate, cu data primei câștigări (`<metal>:<temă>` pentru medalia unei teme, `<metal>-<treaptă>` pentru cele în plus; id-urile de dinainte de v0.13.0 rămân, dar nu se mai arată) |
